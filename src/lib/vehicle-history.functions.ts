  ...
  .handler(async ({ data, context }) => {
    ...
    const [jobs, docs, invoices, claims] = await Promise.all([
      ...
      sb.from("invoices")
        ...
        .in("job_id",
          (await sb.from("jobs").select("id").eq("vehicle_id", data.id)).data?.map(j => j.id) ?? ["00000000-0000-0000-0000-000000000000"]
        )
        .order("invoice_date", { ascending: false, nullsFirst: false }),
      sb.from("insurance_claims")
        .select("id,claim_number,insurer,policy_number,status,approved_amount,effective_date,job_id")
        .in("job_id",
          (await sb.from("jobs").select("id").eq("vehicle_id", data.id)).data?.map(j => j.id) ?? ["00000000-0000-0000-0000-000000000000"]
        )
