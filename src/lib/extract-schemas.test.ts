...
describe("PaymentInput schema", () => {
  ...
  it("rejects negative amounts", () => {
    expect(() => PaymentInput.parse({ ...validPayment, amount: -100 })).toThrow();
  });

  it("rejects amounts over 10M", () => {
    expect(() => PaymentInput.parse({ ...validPayment, amount: 10_000_001 })).toThrow();
  });
  ...
  it("rejects invalid UUID for job_id", () => {
    expect(() => PaymentInput.parse({ ...validPayment, job_id: "not-a-uuid" })).toThrow();
  });

  it("rejects invalid payer_type", () => {
    expect(() => PaymentInput.parse({ ...validPayment, payer_type: "employer" })).toThrow();
  });
...
describe("ExtractSchemas — rejects arbitrary JSON (AR-5 regression)", () => {
  it("rejects an object with random keys for invoice", () => {
    expect(() => ExtractSchemas.invoice.parse({ random: "data", nested: { key: 1 } })).toThrow();
  });
