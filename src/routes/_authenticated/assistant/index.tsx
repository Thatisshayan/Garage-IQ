...
function Assistant() {
  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="space-y-4">
        {history.map((h, i) => (
          <div key={i} className="border border-border rounded-md p-4 bg-card">
            ...
            {h.plan?.explanation && <div className="text-xs text-muted-foreground mt-2 italic">{h.plan.explanation}</div>}
            {h.rows && (
              <div className="mt-3 overflow-x-auto">
                {h.rows.length === 0 ? <div className="text-sm text-muted-foreground">No results.</div> : (
                  <table className="text-xs border border-border w-full">
                    <thead className="bg-background"><tr>{Object.keys(h.rows[0]).map((k) => <th key={k} className="p-2 text-left">{k}</th>)}</tr></thead>
                    <tbody>
                      {h.rows.map((r: any, idx: number) => (
                        <tr key={idx} className="border-t border-border">{Object.values(r).map((v: any, j: number) => <td key={j} className="p-2 font-mono">{typeof v === "object" ? JSON.stringify(v) : String(v ?? "")}</td>)}</tr>
                      ))}
                    </tbody>
                  </table>
                )}
      ...
      <form onSubmit={ask} className="flex gap-2 sticky bottom-4 bg-background pt-2 border-t border-border">
