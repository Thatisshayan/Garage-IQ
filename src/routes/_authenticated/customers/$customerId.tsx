...
function CustomerDetail() {
  ...
  const { data } = useSuspenseQuery({ queryKey: ["customer", customerId], queryFn: () => fn({ data: { id: customerId } }) });
  const c = data.customer;
  if (!c) return <div className="p-8">Not found.</div>;
  return (
    <div className="p-8 space-y-6">
      <Link to="/customers" className="text-xs text-muted-foreground">← Customers</Link>
        ...
        <ul className="space-y-1">
          {data.vehicles.map((v: any) => (
            <li key={v.id} className="border border-border rounded p-2 text-sm">
              {v.year} {v.make} {v.model} · <span className="font-mono text-xs">{v.license_plate || v.vin}</span>
            </li>
          ))}
          {data.vehicles.length === 0 && <li className="text-sm text-muted-foreground">No vehicles</li>}
        </ul>
      </section>
        ...
        <ul className="space-y-1">
          {data.jobs.map((j: any) => (
            <li key={j.id} className="border border-border rounded p-2 text-sm">
              <Link to="/jobs/$jobId" params={{ jobId: j.id }} className="hover:underline">{j.description || "(no description)"}</Link>
