  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <table className="w-full text-sm border border-border rounded-md overflow-hidden">
        <thead className="bg-card"><tr className="text-left"><th className="p-3">Claim #</th><th className="p-3">Insurer</th><th className="p-3">Job</th><th className="p-3">Approved $</th><th className="p-3">Status</th></tr></thead>
        <tbody>
          {data.map((c: any) => (
            <tr key={c.id} className="border-t border-border">
              <td className="p-3 font-mono text-xs">{c.claim_number}</td>
              <td className="p-3">{c.insurer}</td>
              <td className="p-3"><Link to="/jobs/$jobId" params={{ jobId: c.job_id }} className="hover:underline">{c.job?.description ?? c.job_id.slice(0,8)}</Link></td>
              <td className="p-3">{c.approved_amount ?? "—"}</td>
              <td className="p-3">
                <select value={c.status} onChange={async (e) => {
                  try { await upd({ data: { id: c.id, status: e.target.value as any, approved_amount: c.approved_amount } }); toast.success("Updated"); refetch(); }
