...
function FillPage() {
  ...
  const { data: ctx } = useQuery({
    queryKey: ["claim-fill-ctx", jobId],
    queryFn: () => ctxFn({ data: { job_id: jobId } }),
  });
  ...
  const chosen = useMemo(
    () => templates.find((t: any) => t.id === selected),
    [templates, selected],
  );
          ...
          >
            ...
            {templates.map((t: any) => (
              <option key={t.id} value={t.id}>
                {t.name} {t.insurer ? `(${t.insurer})` : ""} —{" "}
                {Object.keys(t.field_map ?? {}).length} fields
              </option>
            ))}
          </select>
          {templates.length === 0 && (
            <p className="text-xs text-muted-foreground mt-2">
              No templates yet.{" "}
              <Link to="/claims/templates" className="text-primary underline">
                Upload one
              </Link>
              .
            </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-sm mt-0.5">
                {Object.keys(preview).length} fields will be filled
              </div>
            </div>
            <Button onClick={download} disabled={busy}>
              <Download className="w-4 h-4 mr-2" /> Download filled PDF
            </Button>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="text-[10px] tick uppercase tracking-wider text-muted-foreground">
                <tr className="text-left">
                  <th className="py-1 pr-4">PDF field</th>
