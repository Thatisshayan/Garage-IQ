    ...
    <div className="p-8 space-y-6">
      ...
      <ul className="space-y-2">
        {data.map((r: any) => (
          <li
            key={r.id}
            className="border border-border rounded-md p-4 bg-card flex justify-between items-start"
          >
            <div>
              <Link
                to="/documents/$docId"
              ...
              <div className="text-xs text-muted-foreground">
                {r.document?.type} · {r.document?.processing_status}
              </div>
              <div className="text-xs mt-1 text-[oklch(0.769_0.188_70.08)] uppercase">
                {r.reason}
              </div>
              {r.details && (
