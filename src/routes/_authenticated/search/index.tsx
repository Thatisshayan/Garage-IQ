    ...
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <form onSubmit={go} className="flex gap-2">
        <Input
          ...
          placeholder="VIN, plate, customer, vendor…"
        />
      ...
      {results && (
        <div className="space-y-6">
          ...
          <Section
            ...
            render={(v: any) => (
              <span>
                ...
                <span className="font-mono text-xs">{v.vin || v.license_plate}</span>
              </span>
            )}
          />
          <Section
          ...
          <Section
            ...
            render={(d: any) => (
              <Link to="/documents/$docId" params={{ docId: d.id }} className="hover:underline">
                {d.file_name} <span className="text-xs text-muted-foreground">{d.type}</span>
              </Link>
            )}
          />
          <Section
            ...
            render={(i: any) => (
              <span>
                {i.vendor} · ${i.total}{" "}
                <span className="text-xs text-muted-foreground">{i.payment_status}</span>
              </span>
            )}
          />
        </div>
  ...
  return (
    <div>
      ...
      <ul className="space-y-1">
        {items.map((it: any) => (
          <li key={it.id} className="border border-border rounded p-2 text-sm">
            {render(it)}
          </li>
        ))}
      </ul>
